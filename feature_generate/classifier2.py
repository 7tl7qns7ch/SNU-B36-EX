...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True):
        ...
        if generalized:
            self.acc_seen, self.acc_unseen, self.H, self.output_seen, self.output_unseen = self.fit()
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            acc_seen = 0
            acc_unseen = 0
            acc_seen, logits_seen = self.val_gzsl(self.test_seen_feature, self.test_seen_label, self.seenclasses)
            acc_unseen, logits_unseen = self.val_gzsl(self.test_unseen_feature, self.test_unseen_label, self.unseenclasses)
            ...
            if H > best_H:
                ...
                best_H = H
                best_logits_seen = logits_seen
                best_logits_unseen = logits_unseen

        return best_seen, best_unseen, best_H, best_logits_seen, best_logits_unseen
    ...
    def val_gzsl(self, test_X, test_label, target_classes): 
        ...
        score = predicted_labels == test_labels
        acc = np.mean(score)
    ...
    def val(self, test_X, test_label, target_classes): 
        ...
        predicted_labels = predicted_label.cpu().detach().numpy()
        test_labels = util.map_label(test_label, target_classes).cpu().detach().numpy()
