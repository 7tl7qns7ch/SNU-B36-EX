...
parser.add_argument('--nz', type=int, default=13, help='size of the latent z vector')
...
parser.add_argument('--G_mode', default='ori', help='ori, trans, split')
parser.add_argument('--D_mode', default='ori', help='ori, trans, split')
parser.add_argument('--att_mode', default=None, help='is lsp')

opt = parser.parse_args()
print(opt)

att_mode = opt.dataset[-2:]
results_name = 'G_' + str(opt.G_mode) + '_D_' + str(opt.D_mode) + '_' + att_mode
...
if opt.att_mode == 'lsp':
    netD = model.Discriminator(opt, floor_thickness=netG.floor_thickness, floor_height=netG.floor_height, receiver_height=netG.receiver_height, x_param=netG.x_param)
else:
...
for epoch in range(opt.nepoch):
    ...
    if epoch % opt.save_every == 0:
        ...
        if opt.gzsl:
            model_path = opt.resultsroot + '/' + results_name + '/netG' + '/netG' + str(epoch) + '.pth'
            model_dir = opt.resultsroot + '/' + results_name + '/netG'
            if os.path.isdir(model_dir) == True:
                pass
            else:
                os.makedirs(model_dir)
            #print(opt.resultsroot, results_name)
            print_output = 'Model saved at ' + opt.resultsroot + '/' + results_name
            print(print_output)
